...
# Add pitch count, batter height adjusted location as pitch crosses plate:
buehrle13$pitch_nbr <- unlist(sapply(rle(buehrle13$date)$lengths,seq))
buehrle13$pz_top <- buehrle13$sz_top-buehrle13$pz
buehrle13$pz_bottom <- buehrle13$sz_bot-buehrle13$pz

# Create dummy variable for men on base (default gives baserunner id):
buehrle13$man_1b<-as.numeric(!is.na(buehrle13$on_1b))
buehrle13$man_2b<-as.numeric(!is.na(buehrle13$on_2b))
buehrle13$man_3b<-as.numeric(!is.na(buehrle13$on_3b))
buehrle13$stand_numeric<-as.numeric(buehrle13$stand=="R")

# Take first month of starts & apply model based cluster on all pitch F/X vars:
buehrle13_4<-subset(buehrle13, buehrle13$date<"2013_05_01")
buehrle13_4.clust_vars<-buehrle13_4[,c(5, 6, 9:11, 13, 15:24, 27, 28, 41,42)]
Sys.time()
clust_april1 <- Mclust(buehrle13_4.clust_vars)
Sys.time()

clust_april1BIC <- mclustBIC(buehrle13_4.clust_vars)
pdf("plots/Buerhle13_clust_april1_BIC.pdf")
plot(clust_april1BIC)
dev.off()
# Attach cluster membership to data
buehrle13_4$class1 <- clust_april1$class
cProb_13_4 <- data.frame(clust_april1$z)
buehrle13_4 <- data.frame(cbind(buehrle13_4, cProb_13_4))
head(buehrle13_4)
